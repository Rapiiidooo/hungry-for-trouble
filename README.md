...
20. **Shelf Destruction:** SHELF CONTROL alternates a cross of lobbed explosives with expanding shockwaves. Find the visible gap or dash through, then free the crew.
...
25. **The Locksmith:** open four locks around the perimeter, enter the central vault and destroy the last backup. Rotating receipt fans alternate with staggered artillery and shockwaves. The only escape follows, then the closing credits.
...
After floor 20, the freed crew and the golden vacuum reward lead to **The Locked Wing**. Five further floors appear only then, including for existing saves that already cleared the core. A short frozen briefing introduces the first key. Collect a key, then approach a matching door to open it automatically. Keys stay with you for that aisle and reset on restart or the next floor. Red triangles, green diamonds, blue circles and yellow squares appear on the pickups, doors, map and compact key ring. Each key turns inside a coloured light beam; while you hold it, its matching doors glow and raise the same beam. Doors carry the key's symbol on both faces and the roof. Closed doors block movement and shots. Explore the branches, collect every key and the crumb quota, then reach checkout; the final vault also requires defeating the Locksmith. Every key is reachable before its own lock, and transport pads cannot bypass the order.
...
node scripts/visual-finish-playtest.mjs
